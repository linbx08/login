    ...
    init(
        ...
        validatedUsername = input.username
            .map { usernameString in
              let usernameRule = ValidationRuleLength(min: 1, max: 100, error: ValidationError(message: "😫"))
                return usernameString.validate(rule: usernameRule)
        }
        
        validatedPassword = input.password
            .map { passwordString in
                let passwordRule = ValidationRuleLength(min: 6, max: 100, error: ValidationError(message: "😫"))
